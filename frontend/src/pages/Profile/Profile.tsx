...
const Profile: React.FC = () => {
    ...
    const fetchUserProfile = async () => {
        try {
            const res = await fetchWithAuth(authContext, `${import.meta.env.VITE_API_URL}/api/users/profile/${userInfo.user_id}/me`, {
                method: "GET",
                headers: {
        ...
        >
            <div className="profile-top-section">
                <div className="h-[250px] w-full rounded-lg bg-[radial-gradient(circle_at_10%_50%,rgba(133,132,212,0.432)_10%,transparent_30%)]">
                    ...
                    {isEditProfileOpen && <EditProfile setIsEditProfileOpen={setIsEditProfileOpen} currentBio={profileData.bio}/>}
                </div>
            </div>
        </motion.div>
    )
