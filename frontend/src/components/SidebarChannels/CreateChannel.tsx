...
const CreateChannel = (props: CreateChannelProps) => {
    const [formData, setFormData] = useState<CreateChannelFormData>({ channelName: "", type: "text" });
    ...
    const handleInputChanged = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        setFormData((prev) => ({
            ...prev,
    ...
    const handleCreateChannelSubmitted = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        ...
        try {
            const res = await fetchWithAuth(authContext, `${import.meta.env.VITE_API_URL}/api/channels`, {
                method: "POST",
                headers: {
            ...
            addToast("error", "Create channel error: " + err);
        } finally {
    ...
    return (
        <div className="fixed z-10">
            ...
            <div className="fixed top-1/2 left-1/2 border-2 border-[var(--color-primary)] transform -translate-x-1/2 -translate-y-1/2 w-[400px] bg-[var(--color-secondary)] rounded-lg flex justify-center items-center">
                <form className="flex flex-col justify-center items-center w-full h-full m-5" onSubmit={handleCreateChannelSubmitted}>
                    <div className="w-full flex justify-between items-center mb-5">
                        <h1 className="text-2xl text-[var(--color-text-primary)]">Create Channel</h1>
                    ...
                    <div className="w-full">
                        <label htmlFor="channel-name" className="block text-[var(--color-text-secondary)]">Channel Name</label>
                        <Input
                            type="text"
                            name="channelName"
                            id="channel-name"
                    ...
                    <div className="w-full mt-2">
                        <label htmlFor="channel-type" className="block text-[var(--color-text-secondary)]">Channel Type</label>
                        <select
                            name="type"
                            id="channel-type"
                            value={formData.type}
                            onChange={handleInputChanged}
                            className="m-0 mt-1 block w-full rounded-md border-[var(--color-text-secondary)] shadow-sm sm:text-sm hover:cursor-pointer"
                            disabled={isSubmitting}
                        >
                    ...
                    <div className="mt-4 w-full">
                        <Button
                            ...
                            className="w-full"
                            disabled={isSubmitting}
                            title={isSubmitting ? "Creating..." : "Create Channel"}
                        />
                    </div>
                </form>
            </div>
